...
describe('render pipeline', () => {
  ...
  it('renderHelix returns invalid-dimensions when dims invalid', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w,h)=>({canvas:{width:w,height:h},_calls:[]}))( );
    // options width/height invalid -> normaliseDimensions returns null
    const r = Mod['renderHelix'](ctx, { width: 0, height: -1 });
    expect(r).toEqual({ ok: false, reason: 'invalid-dimensions' });
  });
  ...
  it('renderHelix draws layers and returns summary with counts; notice optional', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w=400,h=300)=>({
      canvas:{width:w,height:h},_calls:[],fillRect(){this._calls.push(['fillRect'])},
      save(){this._calls.push(['save'])},restore(){this._calls.push(['restore'])},
      arc(){this._calls.push(['arc'])},stroke(){this._calls.push(['stroke'])},
      createRadialGradient(){this._calls.push(['createRadialGradient']); return { addColorStop(){} };},
      fillText(){this._calls.push(['fillText'])},
      set fillStyle(v){ this._calls.push(['set fillStyle', v]); },
      set globalAlpha(v){ this._calls.push(['set globalAlpha', v]); },
      beginPath(){this._calls.push(['beginPath'])}, moveTo(){this._calls.push(['moveTo'])}, lineTo(){this._calls.push(['lineTo'])}
    })) (400, 300);
    const res = Mod['renderHelix'](ctx, { notice: 'Hello' });
    expect(res.ok).toBe(true);
    expect(res.summary).toMatch(/Vesica .* circles · Paths .* \/ Nodes .* · Spiral .* samples · Helix .* ties/);
  ...
  it('renderHelix omits notice draw when empty/whitespace', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w,h)=>({canvas:{width:w,height:h},_calls:[],fillRect(){this._calls.push(['fillRect'])},createRadialGradient(){return { addColorStop(){} };}}))(400,300);
    const res = Mod['renderHelix'](ctx, { notice: '   ' });
    expect(res.ok).toBe(true);
    const hasFillText = ctx._calls.some(c => c[0] === 'fillText');
    expect(hasFillText).toBe(false);
...
describe('fillBackground', () => {
  it('sets fillStyle, paints full rect, and applies radial gradient overlay', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w,h)=>({
      canvas:{width:w,height:h},_calls:[],
      set fillStyle(v){ this._calls.push(['set fillStyle', v]); },
      fillRect(x,y,w,h){ this._calls.push(['fillRect', x,y,w,h]); },
      createRadialGradient(...args){ this._calls.push(['createRadialGradient', ...args]); return { addColorStop(){/*noop*/} }; }
    }))(200,100);
    Mod['fillBackground'](ctx, { width: 200, height: 100 }, '#123456');
    const calls = ctx._calls;
    const firstFillStyleIdx = calls.findIndex(c => c[0] === 'set fillStyle');
...
describe('draw helpers computational returns', () => {
  ...
  it('drawVesicaField returns circle count consistent with rows*cols', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w,h)=>({canvas:{width:w,height:h},_calls:[],arc(){this._calls.push(['arc'])}, beginPath(){}, stroke(){}}))(300,200);
    const cfg = { rows: 3, columns: 4, paddingDivisor: 10, radiusScale: 0.25, strokeDivisor: 50, alpha: 0.5 };
    const stats = Mod['drawVesicaField'](ctx, dims, palette.layers[0], NUM, cfg);
    expect(stats.circles).toBe(3 * 4);
  });
  ...
  it('drawTreeOfLife returns node and path counts', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w,h)=>({canvas:{width:w,height:h},_calls:[],moveTo(){},lineTo(){},stroke(){}}))(300,300);
    const base = Mod['createDefaultGeometry'](NUM).treeOfLife;
    const stats = Mod['drawTreeOfLife'](ctx, dims, palette, NUM, base);
    expect(stats.nodes).toBe(base.nodes.length);
    expect(stats.paths).toBe(base.edges.length);
  ...
  it('drawFibonacciCurve returns samples and marker count computed via interval', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w,h)=>({canvas:{width:w,height:h},_calls:[],arc(){},beginPath(){},stroke(){}}))(300,200);
    const cfg = { ...Mod['createDefaultGeometry'](NUM).fibonacci, sampleCount: 12, markerInterval: 5 };
    const stats = Mod['drawFibonacciCurve'](ctx, dims, palette.layers[3], NUM, cfg);
    expect(stats.samples).toBe(12);
    expect(stats.markers).toBe(3);
  ...
  it('drawHelixLattice returns 2*samples strandPoints and ties count', () => {
    const ctx = (typeof makeMockCtx === 'function' ? makeMockCtx : (w,h)=>({canvas:{width:w,height:h},_calls:[],beginPath(){},moveTo(){},lineTo(){},stroke(){}}))(200,200);
    const cfg = { ...Mod['createDefaultGeometry'](NUM).helix, sampleCount: 11, crossTieCount: 6 };
    const stats = Mod['drawHelixLattice'](ctx, dims, palette, NUM, cfg);
    expect(stats.strandPoints).toBe(22);
    expect(stats.crossTies).toBe(6);
...
// Additional edge/failure scenarios based on diff focus
describe('failure and edge handling', () => {
  it('renderHelix handles extremely small canvas gracefully', () => {
    const tinyCtx = { canvas: { width: 1, height: 1 } };
    const r = Mod['renderHelix'](tinyCtx, {});
    expect(r && typeof r.ok === 'boolean').toBe(true);
  });

  it('mergeTreeGeometry ignores invalid node shapes', () => {
    const NUM = Mod['normaliseNumbers']();
    const base = Mod['createDefaultGeometry'](NUM).treeOfLife;
    const patched = Mod['mergeTreeGeometry'](base, { nodes: [{}, { id: null }, { id: 'x', level: -1 }] });
    expect(Array.isArray(patched.nodes)).toBe(true);
  });

  it('withAlpha returns input for malformed hex', () => {
    expect(Mod['withAlpha']('#12', 0.5)).toBe('#12');
    expect(Mod['withAlpha']('#zzzzzz', 0.5)).toBe('#zzzzzz');
  });
});