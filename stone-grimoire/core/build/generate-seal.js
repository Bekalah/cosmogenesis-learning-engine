...
const SHEM =
  loadJSON(path.join(REG, 'shem72.json')) ||
  Array.from({ length: 72 }, (_, i) => ({ id: i + 1, name: `Shem-${i + 1}` }));
// Simple placeholder names if missing:
const GOE =
  loadJSON(path.join(REG, 'goetia72.json')) ||
  Array.from({ length: 72 }, (_, i) => ({ id: i + 1, name: `Goetia-${i + 1}` }));
const HEB =
  loadJSON(path.join(REG, 'hebrew22.json')) || [
    'א',
    'ב',
    'ג',
    'ד',
...
const PLAN =
  loadJSON(path.join(REG, 'planetary_archangels.json')) || [
...
// datasets (fallbacks if not present)
const REG = path.resolve(ROOT,'../cosmogenesis-learning-engine/registry/datasets');
...
for (let i = 0; i < marks; i++) {
  ...
  if (i % 6 === 0) {
    // every 6th label for legibility
    const idx = Math.floor(i / 2) % 72;
    const label = isAngel
      ? SHEM[idx]?.name || `Shem-${idx + 1}`
      : GOE[idx]?.name || `Goetia-${idx + 1}`;
...
const pts = [];
for (let k = 0; k < 6; k++) {
  pts.push(polar(CX, CY, R_HEX, -90 + k * 60));
}
for (let k = 0; k < 6; k++) {
  const [x1, y1] = pts[k];
...
PLAN.forEach((p, i) => {
  ...
  push(
    `<text x="${px}" y="${py + 8}" font-family="Junicode,serif" font-size="28" text-anchor="middle" fill="${INK}">${esc(p.glyph)}</text>`
  );
...
const SOYGA = 'ABHORSEMEMSOYGA'.split(''); // placeholder letter stream
const turns = 3.5,
  ptsSpiral = 220,
  r0 = 260,
  r1 = 60;
for (let i = 0; i < ptsSpiral; i++) {
  ...
  if (i % 14 === 0) {
    const ch = SOYGA[(i / 14) % SOYGA.length] || 'ᚠ'; // placeholder glyph
  <path d="M ${CX-40} ${CY-120} a 120 120 0 1 0 0 240 a 90 120 0 1 1 0 -240 Z" fill="${C('ink','#141414')}" stroke="${GOLD}" stroke-width="6"/>
  <!-- Cross (Earth/Body) -->
  <line x1="${CX-220}" y1="${CY}" x2="${CX+220}" y2="${CY}" stroke="${GOLD}" stroke-width="10"/>
  <line x1="${CX}" y1="${CY-220}" x2="${CX}" y2="${CY+220}" stroke="${GOLD}" stroke-width="10"/>
