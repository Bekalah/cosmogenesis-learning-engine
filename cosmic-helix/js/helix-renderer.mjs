...
function drawVesicaField(ctx, w, h, color, N) {
  ...
  // Radii derived from 3 keep shapes large enough to breathe; step uses 7
  // so the grid remains gentle and non-distracting.
...
function drawTreeOfLife(ctx, w, h, pathColor, nodeColor, N) {
  ...
  nodes.forEach((n) => {
    ctx.beginPath();
    ctx.arc(n.x, n.y, N.NINE / 3, 0, Math.PI * 2); // node size tied to 9 for calm presence
    ctx.fill();
  });
}
