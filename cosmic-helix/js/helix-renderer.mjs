...
function drawVesicaField(ctx, w, h, color, N) {
  ...
  const radius = Math.min(w, h) / N.THREE; // radius from 3 keeps forms spacious
  const step = radius / N.SEVEN; // grid density tuned by 7 for calm spacing
  // Radii derived from 3 and 7 keep the grid gentle and non-distracting.
...
function drawTreeOfLife(ctx, w, h, pathColor, nodeColor, N) {
  ...
  nodes.forEach((n) => {
    ctx.beginPath();
    ctx.arc(n.x, n.y, N.NINE / 3, 0, Math.PI * 2); // node size tied to 9 for gentle presence
    ctx.fill();
  });
}
