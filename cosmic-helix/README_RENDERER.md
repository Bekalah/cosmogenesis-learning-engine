...
2. A 1440×900 canvas renders four static layers:
   ...
   - **Fibonacci curve** — golden spiral polyline anchored to centre.
   - **Double-helix lattice** — two phase-shifted sine tracks.
3. To customize colours, edit `data/palette.json`. Missing data shows an inline notice with safe defaults.
## Usage
1. Open `index.html` in any modern browser (no server needed).
2. A 1440×900 canvas renders four static layers:
   - **Vesica field** — intersecting circles forming the womb of forms.
...
## ND-safe notes
- Static drawing only; no motion or autoplay.
...
## Design Notes
- Small pure functions and ASCII quotes keep code readable.
- Numerology constants live in `index.html` and are passed to `renderHelix`.
- Module code resides in `js/helix-renderer.mjs`.
...
## Extending
Add new layers by extending `renderHelix` with additional draw functions while preserving the calm visual hierarchy.
...
<<<<<<< codex/update-readme-and-documentation-files
## Design notes
- Static HTML and Canvas keep rendering local and deterministic.
- Geometry routines live in `js/helix-renderer.mjs` with small pure functions and ASCII quotes only.
